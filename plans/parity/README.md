...
```
python3 plans/parity/tools/extract_c_funcs.py    > plans/parity/c_funcs.tsv
python3 plans/parity/tools/extract_py_methods.py > plans/parity/py_methods.tsv
python3 plans/parity/tools/extract_mcp_tools.py  > plans/parity/mcp_tools.tsv
python3 plans/parity/tools/build_matrix.py
```
...
repo. The strict Python release gate is `python/scripts/api_drift_audit.py --strict`;
it excludes comments, strings and extern declarations and includes .pxd helpers.
The older fuzzy matrix is historical and must not be used to claim coverage.
...
```
python3 plans/parity/tools/build_matrix.py --check
```
...
Current implementation evidence is maintained in
`docs/analysis/PYTHON_BINDINGS_IMPLEMENTATION_2026-09-26.md`. The original
review remains a historical baseline. Python exclusions come from the strict
audit; MCP-only exclusions in `overrides.tsv` do not exempt a Python binding.
