@page hydraulics_ref_ch7_advanced_features Chapter 7: Advanced Features
...
over the time period *t* to *t+∆t*:
  ...
  \f[q_{E} = e_{t}W\left( \overline{\overline{Y}} \right)\f]    
  (7-1)

where
  \f$q_{E}\f$ = uniformly distributed evaporation rate along a channel (cfs/ft)
  \f$e_{t}\f$ = potential evaporation rate per unit area over the current time period (cfs/ft<sup>2</sup>)
  \f$\overline{\overline{Y}}\f$ = average depth of flow in the channel over the current time period (ft)
  \f$*W(Y)*\f$ = width of water surface at depth of flow *Y* (ft).
...
@ref hydraulics_ref_ch5_cross_section "Chapter 5" for a particular channel's cross sectional shape.
...
\f[\overline{\overline{Y}} = \frac{\left( {Y(A}_{1}^{t}) + {Y(A}_{2}^{t} \right))}{2}\f]   
(7-2)

where \f$A_{1}^{t}\f$ is the flow area at the upstream end of the channel
previously computed for time *t*, \f$A_{2}^{t}\f$ is the same at the
...
procedures described in @ref hydraulics_ref_ch5_cross_section "Chapter 5".

For dynamic wave modeling the average channel depth is computed as:

\f[\overline{\overline{Y}} = \frac{\left( {\overline{Y}}^{t} + {\overline{Y}}^{t + \mathrm{\Delta}t} \right)}{2}\f]   
(7-3)

where \f$\overline{Y} = \frac{\left( Y_{1} + Y_{2} \right)}{2}\f$. The
*Y<sub>1</sub>* and *Y<sub>2</sub>* values for \f${\overline{Y}}^{t + \mathrm{\Delta}t}\f$ are
...
wave equations. Thus \f$\overline{\overline{Y}}\f$ and therefore *q*<sub>E</sub> can
...
\f[q_{S} = sf_{c}W(\overline{\overline{Y}})\f]                
(7-4)
...
|--------|---|-------------|-------|
| \f[q_{S}\f] | = | uniformly distributed seepage rate per length of conduit | cfs/ft |
...
| \f[\overline{\overline{Y}}\f] | = | average depth of flow in the conduit over the current time period | ft |
...
flow \f$\overline{\overline{Y}}\f$ is limited by the depth at which the
...
\f[q_{L}L\mathrm{\Delta}t \leq \overline{\overline{A}}L\f]    
(7-6)


where \f$\overline{\overline{A}}\f$ is the average flow area over the time
step and *L* is the conduit length. Thus

\f[q_{L} = \min\left( q_{L},\frac{\overline{\overline{A}}}{\mathrm{\Delta}t} \right)\f]   
(7-7)

For kinematic wave analysis the average flow area over the time step
from *t* to *t+∆t* is:

\f[\overline{\overline{A}} = \frac{\left( A_{1}^{t} + A_{2}^{t} \right)}{2}\f]   
(7-8)

where \f$A_{1}^{t}\f$ is the flow area at the upstream end of the conduit
computed for time *t* and \f$A_{2}^{t}\f$ is the same at the downstream end
of the conduit. For dynamic wave analysis:

\f[\overline{\overline{A}} = \frac{\left( {\overline{A}}^{t} + {\overline{A}}^{t + \mathrm{\Delta}t} \right)}{2}\f]   
(7-9)

where
\f${\overline{A}}^{t} = \frac{\left( A\left( Y_{1} \right) + A\left( Y_{2} \right) \right)}{2}\f$
for *Y<sub>1</sub>* and *Y<sub>2</sub>* computed at time *t*, with a similar expression
used for \f${\overline{A}}^{t + \mathrm{\Delta}t}\f$. In the latter case the
...
inflow rate \f$Q_{1}^{t + \Delta t}\f$ to the conduit under kinematic wave
...
\f[Q_{t + \Delta t} = \frac{Q_{t} + {\mathrm{\Delta}Q}_{inertia} + {\mathrm{\Delta}Q}_{pressure} + \mathrm{\Delta}Q_{lateral}}{1 + {\mathrm{\Delta}Q}_{friction}}\f]   
(7-10)

where \f$\mathrm{\Delta}Q_{lateral} = 2.5\overline{U}q_{L}\f$ and all other
...
> 
> Flow depth:               \f$Y = k \cdot Y_{full}/50\f$
> 
...
>    \f$w = x_i - x_{i-1}\f$
>    \f$p = \sqrt{w^2 + (\Delta y)^2}\f$ where \f$\Delta y = |y_i - y_{i-1}|\f$
> 
> 4. If the segment is completely submerged (\f$Y > \max(y_{i-1}, y_i)\f$) compute its area \f$a\f$ as:
>    \f$a = w(Y - (y_{i-1} + y_i)/2)\f$
>    Otherwise let \f$\alpha = (Y - \min(y_{i-1}, y_i))/\Delta y\f$ and set \f$a = \alpha^2 w \Delta y/2\f$.
> 
> 5. Adjust the width and wetted perimeter for partial submergence:
>    \f$w = \alpha w\f$;  \f$p = \alpha p\f$
> 
> 6. Update the table entries for area and top width:
>    \f$A_{tbl}[k] = A_{tbl}[k] + a\f$;  \f$W_{tbl}[k] = W_{tbl}[k] + w\f$
> 
> 7. Update the area and wetted perimeter of the current compound segment:
>    \f$A_{sum} = A_{sum} + a\f$;  \f$P_{sum} = P_{sum} + p\f$
> 
...
>    \f$K = K + (1.486/n_i) A_{sum} (A_{sum}/P_{sum})^{2/3}\f$
...
>     \f$R_{tbl}[k] = ((n_C K)/(1.486 A_{tbl}[k]))^{3/2}\f$
...
negative flow. This modifies the \f$\sum_{}^{}Q^{t + \mathrm{\Delta}t}\f$
...
\f[\frac{\partial A}{\partial t} + \frac{\partial Q}{\partial x} + q_{L} = 0\f]                 
(7-11)

Carrying the *q*<sub>L</sub> term over into the original finite difference form
of this equation (Equation 4-7) produces:

\f[\frac{(1 - \theta)\left( A_{1}^{t + \Delta t} - A_{1}^{t} \right) + \theta\left( A_{2}^{t + \Delta t} - A_{2}^{t} \right)}{\Delta t} + \frac{(1 - \phi)\left( Q_{2}^{t} - Q_{1}^{t} \right) + \phi\left( Q_{2}^{t + \Delta t} - Q_{1}^{t + \Delta t} \right)}{L} + q_{L} = 0\f]   
(7-12)

where all notation was defined previously in Section 4.2. After
substituting the Manning equation \f$Q = \beta\Psi(A)\f$ into this
expression the same non-linear equation for \f$A_{2}^{t + \Delta t}\f$
results as before (equation 4-8):

\f[\beta\Psi\left( A_{2}^{t + \Delta t} \right) + C1A_{2}^{t + \Delta t} + C2 = 0\f]   
(7-13)

with *C1* given by Equation 4-9 and *C2* by Equation 4-10 but with the
additional term \f$q_{L}\frac{L}{\phi}\f$ added to it.
...
-
\f[Q_{EN} = e_{t}f_{E}A_{SN}\left( Y^{t} \right)\f]           
(7-14)

where

  \f$Q_{EN}\f$ = evaporation loss rate from a storage unit node (cfs)

  *t* = starting time for the current computational time period (sec)

  \f$e_{t}\f$ = potential evaporation rate per unit area at time *t* (cfs/ft<sup>2</sup>)
...
\f[q_{SN} = K_{S}f_{C}\left\lbrack 1 + \frac{\left( \psi_{S} + d \right)\theta_{d}}{F} \right\rbrack\f]   
(7-15)
...
where
  ...
  *d* = depth of stored water above the area undergoing seepage (ft)
  ...
  \f$\psi_{S}\f$ = soil capillary suction head (ft)
...
quantity \f$\psi_{s}\f$ in the original formulation is replaced with
\f$\psi_{s} + d\f$. Volume I also provides guidance on selecting values of
*K*<sub>S</sub>, \f$\psi_{s}\f$, and an initial *θ*<sub>d</sub> based on soil type. If
either \f$\psi_{s}\f$ or *θ*<sub>d</sub> are 0 then SWMM assumes a constant seepage
...
\f[Q_{SN} = q_{btm}\left( d_{btm} \right)A_{btm} + q_{side}\left( d_{side} \right)A_{side}\f]   
(7-16)
...
|---|---|---|
| for \f$Y^{t} < d_{\min}\f$ | \f$0\f$ | (7-17) |
| for \f$d_{\min} \leq Y^{t} \leq d_{\max}\f$ | \f$\frac{\left( Y^{t} - d_{\min} \right)}{2}\f$ | |
| for \f$Y^{t} > d_{\max}\f$ | \f$Y^{t} - \frac{\left( d_{\max} - d_{\min} \right)}{2}\f$ | |
...
-
\f[A_{side} = min\left\{ A\left( Y^{t} \right),\ A\left( d_{\max} \right) \right\} - A_{btm}\f]   
(7-18)
...
\f[Q_{LN} = min\left\{ Q_{LN},\frac{V_{N}\left( Y^{t} \right)}{\mathrm{\Delta}t} \right\}\f]   
(7-20)
...
\f$\sum_{}^{}Q^{t + \mathrm{\Delta}t}\f$ term of Equation 3-15a (i.e., is
...
\f[\mathrm{\Delta}H_{L} = K_{m,i}\frac{U_{i}^{2}}{2g}\f]      
(7-21)


where \f$\mathrm{\Delta}H_{L}\f$ is the minor head loss (ft), *K*<sub>m,i</sub> is
...
\f[\frac{\partial Q}{\partial t} + \frac{\partial\left( \frac{Q^{2}}{A} \right)}{\partial x} + gA\frac{\partial H}{\partial x} + gAS_{f} - U\frac{q_{L}}{2} + gAh_{L} = 0\f]   
(7-22)


where \f$h_{L} = \frac{\sum_{i = 1}^{3}{K_{m,i}U_{i}^{2}}}{(2gL)}\f$ with
...
\f[\frac{\mathrm{\Delta}Q}{\mathrm{\Delta}t} = 2\overline{U}\frac{\mathrm{\Delta}\overline{A}}{\mathrm{\Delta}t} + {\overline{U}}^{2}\frac{\left( A_{2} - A_{1} \right)}{L} - g\overline{A}\frac{\left( H_{2} - H_{1} \right)}{L} - g\eta^{2}\frac{Q\left| \overline{U} \right|}{{\overline{R}}^{4/3}} + 2.5\overline{U}q_{L} - \frac{Q\sum_{i = 1}^{3}{K_{m,i}\left| U_{i} \right|}}{2L}\f]   
(7-23)
...
\f[Q_{t + \Delta t} = \frac{Q_{t} + {\mathrm{\Delta}Q}_{inertia} + {\mathrm{\Delta}Q}_{pressure} + \mathrm{\Delta}Q_{lateral}}{1 + {\mathrm{\Delta}Q}_{friction} + \mathrm{\Delta}Q_{loss}}\f]   
(7-24)

where

\f[\mathrm{\Delta}Q_{loss} = \frac{\mathrm{\Delta}t}{2L}\sum_{i = 1}^{3}{K_{m,i}\left| U_{i} \right|}\f]   
(7-25)
...
\f[U = 1.318C_{HW}R_{full}^{0.63}S_{f}^{0.54}\f]              
(7-26)
...
\f[S_{f} = \frac{0.6|U|^{0.852}Q}{C_{HW}^{1.852}A_{full}R_{full}^{1.667}}\f]   
(7-27)
...
\f[{\mathrm{\Delta}Q}_{friction} = 0.6g\frac{\left| \overline{U} \right|^{0.852}\Delta t}{C_{HW}^{1.852}R_{full}^{1.667}}\f]                      
(7-28)

where \f$\overline{U} = \frac{Q^{last}}{A_{full}}\f$.
...
dimensionless friction factor. Noting that \f$D = 4R_{full}\f$ for a
circular pipe allows this equation to be expressed in a form similar to
the Manning formula:


\f[S_{f} = \frac{f|U|Q}{8gA_{full}R_{full}}\f]                
(7-30)
...
\f[{\mathrm{\Delta}Q}_{friction} = \frac{f\left| \overline{U} \right|\Delta t}{8R_{full}}\f]                      
(7-31)
...
where \f$Re = \frac{D\left| \overline{U} \right|}{\mu}\f$ with *μ* being the
...
\f[f = \frac{0.25}{\left\lbrack \log\left( \frac{\epsilon}{3.7D} + \frac{5.74}{{Re}^{0.9}} \right) \right\rbrack^{2}}\f]                      
(7-33)


where \f$\epsilon\f$ is the equivalent surface roughness height (ft) of the
...
4000 (which will also depend on \f$\frac{\epsilon}{D}\f$).
...
\f[\left( \frac{1.486}{n} \right)^{2}R_{full}^{4/3}S_{O} = \left( 1.318C_{HW}R_{full}^{0.63}S_{O}^{0.54} \right)^{2}\f]                      
(7-34)


where *S*<sub>O</sub> is the pipe's bottom slope (ft/ft), *R*<sub>full</sub> is in feet,
and *n* has units of sec/m<sup>1/3</sup>. Expressing *R*<sub>full</sub> as \f$\frac{D}{4}\f$
and solving for *n* gives:

\f[n = \frac{1.067\left( \frac{D}{S_{O}} \right)^{0.04}}{C_{HW}}\f]                      
(7-35)


Doing the same for the Darcy-Weisbach formula produces:

\f[\left( \frac{1.486}{n} \right)^{2}R_{full}^{4/3}S_{O} = \frac{2gDS_{O}}{f(\epsilon,\infty)}\f]                      
(7-36)

where

\f[f(\epsilon,\infty) = \frac{0.25}{\left\lbrack \log\left( \frac{\epsilon}{3.7D} \right) \right\rbrack^{2}}\f]                      
(7-37)


Expressing *R<sub>full</sub>* as \f$\frac{D}{4}\f$ and solving for *n* gives:

\f[n = \sqrt{\frac{f(\epsilon,\infty)}{185}}D^{1/6}\f]                      
(7-38)
...
\f[{\mathrm{\Delta}Q}_{friction}\f] in the iterative flow updating Equation
3-14. For free surface flow it uses the Manning form of
\f[{\mathrm{\Delta}Q}_{friction}\f] (Equation 3-14c) with an *n*-value given
...
\f$A_{full}\sqrt{Y_{full}}\f$ where *A<sub>full</sub>* is the full cross-section area
...
\f[\frac{H_{1} - Z_{1}}{Y_{full}} = \frac{E_{C}}{Y_{full}} + K_{I}\left\lbrack \frac{Q_{IC}}{A_{full}\sqrt{Y_{full}}} \right\rbrack^{M_{I}} + ScfS_{O}\f]   
(7-39)


while the form 2 equation is:

\f[\frac{H_{1} - Z_{1}}{Y_{full}} = K_{I}\left\lbrack \frac{Q_{IC}}{A_{full}\sqrt{Y_{full}}} \right\rbrack^{M_{I}}\f]   
(7-40)
...
\f$g^{\frac{- M_{I}}{2}}\f$ embedded in it so that equations 7-39 and 7-40
...
\f[Q_{IC} = A_{full}\sqrt{Y_{full}}\left( \frac{H_{1} - Z_{1}}{K_{I}Y_{full}} \right)^{\frac{1}{M_{I}}}\f]   
(7-41)
...
\f[U_{C}^{2} = g\frac{A\left( Y_{C} \right)}{W\left( Y_{C} \right)}\f]   
(7-43)


and from Equation 5-29:

\f[Q_{IC} = A\left( Y_{C} \right)\sqrt{g\frac{A\left( Y_{C} \right)}{W\left( Y_{C} \right)}}\f]   
(7-44)


The area *A* and top width *W* values in these equations are evaluated
at flow depth *Y*<sub>C</sub> using the methods described in @ref hydraulics_ref_ch5_cross_section "Chapter 5" that
...
\f[\frac{Y_{C}}{Y_{full}} = \frac{H_{1} - Z_{1} - \frac{Y_{HC}}{2}}{Y_{full}} - K_{I}\left\lbrack \frac{A\left( Y_{C} \right)}{A_{full}}\sqrt{g\frac{Y_{HC}}{Y_{full}}} \right\rbrack^{M_{I}} - ScfS_{O}\f]   
(7-45)


where \f$Y_{HC}\f$ is the critical hydraulic depth defined as
\f$\frac{A\left( Y_{C} \right)}{W\left( Y_{C} \right)}\f$. This equation is
...
\f[\frac{H_{1} - Z_{1}}{Y_{full}} = c_{I}\left\lbrack \frac{Q_{IC}}{A_{full}\sqrt{Y_{full}}} \right\rbrack^{2} + y_{I} + ScfS_{O}\f]   
(7-46)
...
\f$\frac{1}{g}\f$ embedded in it so that equation 7-46 is dimensionally
consistent. Solving this expression for *Q*<sub>IC</sub> results in:

\f[Q_{IC} = \left\lbrack \left( \frac{1}{c_{I}} \right)\left( \frac{H_{1} - Z_{1}}{Y_{full}} - y_{I} - ScfS_{O} \right) \right\rbrack^{\frac{1}{2}}A_{full}\sqrt{Y_{full}}\f]   
(7-47)
...
\f$\frac{Q_{IC}}{\left( A_{full}\sqrt{Y_{full}} \right)}\f$ above 4.0.
Converting this into a condition on *H<sub>1</sub>* results in:

\f[H_{1} > {H_{IS} = Z}_{1} + Y_{full}\left( 16c_{I} + y_{I} + ScfS_{O} \right)\f]   
(7-48)
...
\f$\frac{Q_{IC}}{\left( A_{full}\sqrt{Y_{full}} \right)}\f$ values below
...
\f[H_{1} < {H_{IU} = Z}_{1} + 0.95Y_{full}\f]                 
(7-49)
...
\f[Q_{IC} = Q_{IC}\left( H_{IU} \right) + \left( Q_{IC}\left( H_{IS} \right) - Q_{IC}\left( H_{IU} \right) \right)\frac{\left( H_{1} - H_{IU} \right)}{\left( H_{IS} - H_{IU} \right)}\f]   
(7-50)

where \f$Q_{IC}\left( H_{IU} \right)\f$ is the flow from the unsubmerged
equation for a head of *H*<sub>IU</sub> and \f$Q_{IC}\left( H_{IS} \right)\f$ is the
...
\f[\frac{dQ_{IC}}{dH_{1}} = \left\{ \begin{matrix}                                               
...
Unlike the other weirs discussed in @ref hydraulics_ref_ch6_pumps_regulators "Chapter 6", a roadway weir has
...
\f[Q_{c} = Q\left\{ R_{f}E_{0} + R_{s}(1 - E_{0}) \right\}\f]   
(7-53)
...
\f[R_{s} = 1/\left\{ 1 + 0.15V^{1.8}/(S_{x}L^{2.3}) \right\}\f]   
(7-55)
...
|---|---|
| P-50 | \f$V_{0} = 2.22 + 4.03L - 0.65L^{2} + 0.06L^{3}\f$ |
| P-50x100 | \f$V_{0} = 0.74 + 2.44L - 0.27L^{2} + 0.02L^{3}\f$ |
| P-30 | \f$V_{0} = 1.76 + 3.12L - 0.45L^{2} + 0.03L^{3}\f$ |
| Curved Vane | \f$V_{0} = 0.30 + 4.85L - 1.31L^{2} + 0.15L^{3}\f$ |
| 45° Tilt Bar | \f$V_{0} = 0.99 + 2.64L - 0.36L^{2} + 0.03L^{3}\f$ |
| 30° Tilt Bar | \f$V_{0} = 0.51 + 2.34L - 0.20L^{2} + 0.01L^{3}\f$ |
| Reticuline | \f$V_{0} = 0.28 + 2.28L - 0.18L^{2} + 0.01L^{3}\f$ |
...
\f[Q_{c} = Q\left\{ 1 - {(1 - min(1,\ L/L_{T})\ )}^{1.8} \right\}\f]   
(7-56)
...
\f[L_{T} = 0.6Q^{0.42}S_{L}^{0.3}{(nS_{e})}^{- 0.6}\f]        
(7-57)
...
where:
  ...
  *S*<sub>e</sub>    =  \f[S_{x} + (\frac{a}{W)E_{0}}\f]
...
\f[Q = (\frac{0.56}{n)S_{x}^{1.67}S_{L}^{0.5}T^{2.67}}\f]     
(7-58)
...
\f[T = \left\lbrack \frac{Qn}{0.56S_{x}^{1.67}S_{L}^{0.5}} \right\rbrack^{0.375}\f]   
(7-59)

If the street has a uniform cross slope (\f$a = 0\f$ in Figure 7-12) then
Equation 7-59 can be used to derive the following expression for*E<sub>0</sub>*:

\f[E_{0} = 1 - {(1 - \frac{W}{T})}^{2.67}\f]                  
(7-60)


where *T* is evaluated at a given flow rate *Q*.

For a compound street cross-section with depressed curb (\f$a > 0\f$ in
Figure 7-12), HEC-22 provides the following equation for *E<sub>0</sub>*:

\f[E_{0} = \frac{1}{1 + \frac{\frac{S_{W}}{S_{X}}}{\left\lbrack 1 + \frac{\frac{S_{W}}{S_{X}}}{\left( \frac{T}{W} - 1 \right)} \right\rbrack^{2.67} - 1}}\f]   
(7-61)


where \f$S_{W} = S_{X} + \frac{a}{W}\f$. It is not possible to solve
...
7-67:
  ...
  *A*<sub>O</sub>    =  \f[LWf_{O}\f]

  *d*       =  \f[d_{i} - (\frac{W}{2)S_{W}}\f]
...
This results in weir flow for depths below \f$1.79\frac{A_{O}}{L_{W}}\f$ and
...
\f$L_{W} = L + 1.8W\f$ where *L* = opening length and *W* = width of the
depressed gutter. The values used in Equation 7-67 for orifice flow are
\f$C_{O} = 0.67\f$ and \f$A_{O} = hL\f$ where *h* is the height of the opening.
...
|----|----|-----|
| Horizontal | ![HorizontalThroat.bmp](hydraulics/media/media/image72.png) | \f$d = d_{i} - \frac{h}{2}\f$ |
| Inclined | ![InclinedThroat.bmp](hydraulics/media/media/image73.png) | \f$d = d_{i} - 0.7071(\frac{h}{2})\f$ |
| Vertical | ![VerticalThroat.bmp](hydraulics/media/media/image74.png) | \f$d = d_{i}\f$ |
...
interpolation formula:
  ...
  \f[Q_{c} = (1 - r)Q_{weir} + rQ_{orif}\f]                     
  (7-68)


where *Q*<sub>weir</sub> is weir flow capture at depth *h*, *Q*<sub>orif</sub> is orifice
flow capture at depth *1.4h* and \f$r = \frac{(d - h)}{(0.4h)}\f$.
...
\f$d \leq 0.2\f$ feet while orifice flow occurs for \f$d \geq 0.4\f$ feet. In
...
capture at depth 0.4 and \f$r = \frac{(d - 0.2)}{0.2}\f$.
...
\f[E_{0} = \frac{1.486\sqrt{S_{L}}{(yW)}^{1.67}}{nQ{(W + 2y)}^{0.67}}\f]   
(7-69)
