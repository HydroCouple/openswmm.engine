@page hydrology_ref_ch3_surface_runoff Chapter 3: Surface Runoff
...
\f[\frac{\partial d}{\partial t} = i - e - f - q\f] (3-1)
...
\f[Q = \frac{1.49}{n}S^{1/2}R_{x}^{2/3}A_{x}\f] (3-2)
...
follows that \f$A_{x} = W(d - d_{s})\f$ and \f$R_{x} = d - d_{s}\f$.
Substituting these expressions into Equation 3-2 gives:

\f[Q = \frac{1.49}{n}WS^{1/2}\left( d - d_{s} \right)^{5/3}\f] (3-3)
...
\f[q = \frac{1.49WS^{1/2}}{A\ n}\left( d - d_{s} \right)^{5/3}\f] (3-4)

Substituting this equation into the original mass balance relation 3-1
results in:

\f[\frac{\partial d}{\partial t} = i - e - f - \alpha\left( d - d_{s} \right)^{5/3}\f] (3-5)

where *α* is defined as:

\f[\alpha = \frac{1.49WS^{1/2}}{A\ n}\f] (3-6)
...
\f[\frac{\partial d}{\partial t} = i - e - f\f] (3-7)
...
- The contribution from snowmelt will vary by subarea. See @ref hydrology_ref_ch6_snowmelt "Chapter 6" for
  details.
...
\f[\alpha_{P} = \frac{1.49WS^{1/2}}{A_{1}n_{P}}\f] for the pervious subarea A1 (3-8)

\f[\alpha_{I} = \frac{1.49WS^{1/2}}{\left( A_{2} + A_{3} \right)n_{I}}\f] for both impervious subareas A2 and A3 (3-9)
...
2. For each subcatchment, retrieve its current precipitation rate *i* and evaporation rate *e* from the data sources described in @ref hydrology_ref_ch2_meteorology "Chapter 2".

3. For each subarea within each subcatchment:
   a. If snow melt is being simulated, use the procedures described in @ref hydrology_ref_ch6_snowmelt "Chapter 6" to adjust the precipitation rate *i* to reflect any snow accumulation (which decreases *i*) or snow melt (which increases *i*).
   ...
   c. If the subarea is pervious, then determine the infiltration rate *f* using the methods described in @ref hydrology_ref_ch4_infiltration "Chapter 4" and if groundwater is being simulated consider the possible reduction in *f* that can occur due to fully saturated conditions (see @ref hydrology_ref_ch5_groundwater "Chapter 5"). Otherwise set *f* = 0.
...
b. Use a standard fifth-order Runge-Kutta integration routine with adaptive step size control (Press et al., 1992) to solve the equivalent of Equation 3-5,
   \f[\frac{\partial d}{\partial t} = i_x - \alpha d_x^{5/3}\f]
...
2.  For subcatchments where a fraction *f* of impervious runoff is
    ...
    \f$f\left( q_{2}A_{2} + q_{3}A_{3} \right)/A_{1}\f$ added to it, where
...
3.  For subcatchments where a fraction *f* of the pervious runoff is
    ...
    \f$q_{1}A_{1}/A_{2}\f$ added to it.
...
coefficient \f$\alpha = \frac{1.49WS^{1/2}}{A\ n}\f$ and the excess in
...
Each equation has the form \f$EIA = a{TIA}^{b}\f$ where the coefficients *a*
...
as \f$\frac{A}{L^{2}}\f$ where A is the watershed area and L is the length
of the watershed's main drainage channel (not necessarily the length of
overland flow). The shape factor Y for the idealized watershed is \f$W/L\f$.
...
\f$\frac{S_{o}L}{(A/YL + YL)}\f$ where S~o~ is the slope of the drainage
...
\f[V = C\left( P - d_{S} \right)\f] (3-13)
...
consideration, then the groundwater routines (@ref hydrology_ref_ch5_groundwater "Chapter 5") might be
invoked to allow the water table to rise to the surface, or the maximum
infiltration volume option used (@ref hydrology_ref_ch4_infiltration "Chapter 4"). When calibrating for more
...
\f[Q = \left\lbrack Ci + (1 - C)max(0,\ i - f) \right\rbrack A\f] (3-17)
...
4.  Use the Horton infiltration option (discussed in @ref hydrology_ref_ch4_infiltration "Chapter 4") and let
...
2.  Select the Curve Number method for computing infiltration (see
    @ref hydrology_ref_ch4_infiltration "Chapter 4") and use the same curve number that one would use with the
    SCS method.
...
\f[R = \frac{(P - Ia)^{2}}{P - Ia + S}\f] (3-18)

where

\f[S = \frac{1000}{CN} - 10\f] (3-19)
...
The details are described in @ref hydrology_ref_ch7_rdii "Chapter 7". Any location within the drainage
