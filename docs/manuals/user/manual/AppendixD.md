@page user_manual_appendix_d APPENDIX D - COMMAND LINE SWMM
...
For an XPOLYGON curve, each data line supplies one vertex of the cross-section boundary as either an (X, Y) pair or an (X, Y, Bulge) triple; every vertex in a given curve must use the same number of values. Coordinates may be entered in any convenient units and any convenient placement (the invert is located automatically as the boundary's lowest point). The boundary may be traced in either direction — a clockwise chain is reoriented automatically, with each Bulge sign flipped to keep the same physical arcs — but it must not cross itself. A Bulge of 0, or an omitted Bulge, connects a vertex to the next one with a straight segment. A non-zero Bulge connects them with a circular arc instead, using the same convention as the DXF file format: Bulge = tan(theta/4), where theta is the arc's included angle in radians, positive when the arc sweeps counterclockwise from this vertex to the next and negative when it sweeps clockwise. On a boundary traced counterclockwise, a positive Bulge bows outward, enlarging the cross-section, while a negative Bulge cuts inward (a concave notch). The magnitude of Bulge may not exceed 1; a Bulge of exactly 1 sweeps a half circle, and any larger arc must be split into two vertices.
