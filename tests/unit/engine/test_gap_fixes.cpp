...
// ===========================================================================
// Regression: storage_getDepth()'s Newton branch (node.c, FUNCTIONAL shape
// with a0 != 0, a1 != 0, a2 not in {0, 1} — the only case, along with
// CONICAL/PYRAMIDAL, legacy solves via findroot_Newton rather than a closed
// form). StorageFunctionalNonlinear above uses a2=1, which both legacy and
// this engine solve in closed form and so never actually exercises Newton.
// ===========================================================================

TEST(NodeGetDepth, StorageFunctionalNewtonBranch) {
    NodeData nodes;
    nodes.resize(1);
    nodes.type[0] = NodeType::STORAGE;
    nodes.full_depth[0] = 20.0;
    ...
    S.c[ur] = 50.0;   // a0
    S.a[ur] = 10.0;   // a1
    S.b[ur] = 2.0;    // a2 (not 0 or 1 -> forces Newton)

    // V = a0*d + a1/(a2+1)*d^(a2+1) = 50*d + (10/3)*d^3
    double d_test = 3.0;
    double v_at_3 = 50.0 * d_test + (10.0 / 3.0) * std::pow(d_test, 3.0);

    double d = node::getDepth(nodes, 0, v_at_3, nullptr, /*unit_sys=*/0, &subs);
    EXPECT_NEAR(d, d_test, 0.01);
}

TEST(NodeGetDepth, StorageFunctionalNewtonBranch_SIUnits) {
    // Same geometry as StorageFunctionalNewtonBranch, solved with
    // unit_sys=1 (SI). getVolume()/getSurfArea() take/return depth and
    // volume/area in INTERNAL (ft, ft3, ft2) units; the Newton iterate and
    // target volume inside getDepth() are USER-unit. Pre-fix, the Newton
    // callback mixed the two unit systems, which was silent under US units
    // (UCF(LENGTH) == UCF(VOLUME) == 1.0) but broke the round trip under SI
    // (UCF(LENGTH) = 0.3048, UCF(VOLUME) = 0.02832). Compare against a
    // getVolume() round trip rather than a hand-derived SI value, so this
    // test doesn't depend on independently re-deriving the UCF arithmetic.
    NodeData nodes;
    nodes.resize(1);
    nodes.type[0] = NodeType::STORAGE;
    nodes.full_depth[0] = 20.0;  // internal ft
    nodes.full_volume[0] = 0.0;
    openswmm::NodeSubtypes subs;
    const int sr = subs.set_node_type(nodes, 0, NodeType::STORAGE);
    auto& S = subs.storages; const auto ur = static_cast<std::size_t>(sr);
    ...
    double d_ft = 20.0 / 3.0;  // mid-range internal depth, well under fullDepth
    double v_ft3 = node::getVolume(nodes, 0, d_ft, nullptr, /*unit_sys=*/1, &subs);
    ASSERT_GT(v_ft3, 0.0);

    double d_back = node::getDepth(nodes, 0, v_ft3, nullptr, /*unit_sys=*/1, &subs);
    EXPECT_NEAR(d_back, d_ft, 0.01);
}

TEST(NodeGetDepth, StorageFunctionalNewtonBranch_NonZeroIndex) {
    // Guards against reintroducing the legacy "storage subIndex passed as
    // node index" bug: node 0 is a JUNCTION, node 1 is the STORAGE node
    // under test, so STOR1's own storage subIndex (0) differs from its node
    // index (1).
    NodeData nodes;
    nodes.resize(2);
    nodes.type[0] = NodeType::JUNCTION;
    nodes.full_depth[0] = 5.0;
    nodes.type[1] = NodeType::STORAGE;
    nodes.full_depth[1] = 20.0;
    nodes.full_volume[1] = 0.0;
    openswmm::NodeSubtypes subs;
    const int sr = subs.set_node_type(nodes, 1, NodeType::STORAGE);
    auto& S = subs.storages; const auto ur = static_cast<std::size_t>(sr);
    S.curve[ur] = -1;
    S.c[ur] = 50.0;
    S.a[ur] = 10.0;
    ...
    double d = node::getDepth(nodes, 1, v_at_3, nullptr, /*unit_sys=*/0, &subs);
    EXPECT_NEAR(d, d_test, 0.01);
}
