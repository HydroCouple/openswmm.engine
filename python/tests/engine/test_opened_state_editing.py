...
class TestOpenedStateNodeEditing(EngineSolverCase):
    def test_add_node_round_trip(self):
        ...
        self.assertEqual(len(opened_solver.nodes), before + 1)
        self.assertEqual(opened_solver.nodes.get_index("PY_TEST_NODE"), before)
        self.assertEqual(node.id, "PY_TEST_NODE")

    def test_pop_last_node_undoes_add(self):
        opened_solver = self.opened_solver()
        before = len(opened_solver.nodes)
        opened_solver.nodes.add("PY_TMP_NODE", NodeType.JUNCTION)
        opened_solver.nodes.pop_last("PY_TMP_NODE")
        self.assertEqual(len(opened_solver.nodes), before)
        with self.assertRaises(KeyError):
            opened_solver.nodes.get_index("PY_TMP_NODE")

    def test_pop_last_node_wrong_tail_raises(self):
        opened_solver = self.opened_solver()
        opened_solver.nodes.add("PY_REAL_TAIL", NodeType.JUNCTION)
        # v1 raises BadIndexError (also IndexError) instead of returning rc.
        with self.assertRaises((BadIndexError, IndexError)):
            opened_solver.nodes.pop_last("NOT_THE_TAIL")
        self.assertGreaterEqual(opened_solver.nodes.get_index("PY_REAL_TAIL"), 0)


class TestOpenedStateLinkEditing(EngineSolverCase):
    def test_add_link_round_trip(self):
        opened_solver = self.opened_solver()
        ...
        self.assertEqual(len(opened_solver.links), before + 1)
        self.assertEqual(opened_solver.links.get_index("PY_TEST_LINK"), before)
        self.assertEqual(link.id, "PY_TEST_LINK")

    def test_pop_last_link_undoes_add(self):
        opened_solver = self.opened_solver()
        ...
        self.assertEqual(len(opened_solver.links), before)
        with self.assertRaises(KeyError):
            opened_solver.links.get_index("PY_TMP_LINK")

    def test_pop_last_link_wrong_tail_raises(self):
        opened_solver = self.opened_solver()
        ...
        self.assertGreaterEqual(
            opened_solver.links.get_index("PY_REAL_LINK_TAIL"), 0)


class TestOpenedStateSubcatchmentEditing(EngineSolverCase):
    def test_add_subcatchment_round_trip(self):
        opened_solver = self.opened_solver()
        before = len(opened_solver.subcatchments)
        sub = opened_solver.subcatchments.add("PY_TEST_SC")
        self.assertEqual(len(opened_solver.subcatchments), before + 1)
        self.assertEqual(opened_solver.subcatchments.get_index("PY_TEST_SC"), before)
        self.assertEqual(sub.id, "PY_TEST_SC")


class TestOpenedStateGageEditing(EngineSolverCase):
    def test_add_gage_round_trip(self):
        opened_solver = self.opened_solver()
        before = len(opened_solver.gages)
        gage = opened_solver.gages.add("PY_TEST_GAGE")
        self.assertEqual(len(opened_solver.gages), before + 1)
        self.assertEqual(opened_solver.gages.get_index("PY_TEST_GAGE"), before)
        self.assertEqual(gage.id, "PY_TEST_GAGE")


class TestOpenedStateInflowEditing(EngineSolverCase):
    """OPENED state must accept inflow additions."""

    def test_add_external_inflow_in_opened_state(self):
        opened_solver = self.opened_solver()
        self.assertGreater(len(opened_solver.nodes), 0)
        opened_solver.inflows.add_external(0, "FLOW", baseline=0.5)
        self.assertGreaterEqual(opened_solver.inflows.external_count, 1)

    def test_add_dwf_in_opened_state(self):
        opened_solver = self.opened_solver()
        self.assertGreater(len(opened_solver.nodes), 0)
        opened_solver.inflows.add_dwf(0, "FLOW", avg_value=1.5)

    def test_add_rdii_in_opened_state(self):
        opened_solver = self.opened_solver()
