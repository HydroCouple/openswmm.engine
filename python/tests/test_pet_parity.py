...
class TestPetParity(unittest.TestCase):
    def test_total_evap_parity(self):
        ...
        self.assertAlmostEqual(engine_total, legacy_total,
                               delta=_PARITY_RTOL * abs(legacy_total))

    def test_groundwater_model_responds_to_prescription(self):
        ...
        if not (os.path.isfile(_GW_LEGACY_INP) and os.path.isfile(_GW_ENGINE_INP)):
            self.skipTest("groundwater parity models not present")
        ...
        for total_fn in (legacy_total_evap, engine_total_evap):
            ...
            self.assertTrue(suppressed <= baseline or baseline == 0.0)
            self.assertGreaterEqual(boosted, baseline)
            # Prescribing zero must fully suppress potential evaporation.
            self.assertAlmostEqual(suppressed, 0.0, delta=1e-9)
