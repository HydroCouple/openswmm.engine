...
.. list-table::
   ...
   * - ``set_vertex_z_bulk(z)``
     - Set **every** vertex ground elevation in one call (one mesh rescan
       instead of one per vertex; GIL released). ``len(z)`` must equal
       ``n_vertices``.
...
.. list-table::
   ...
   * - ``get_vertex_coupling_cd(vertex_idx)`` /
     ...
     - Coupling discharge coefficient (``[2D_VERTEX_NODE_MAP]`` CD
       column; default 0.65, must be > 0). Persisted by the ``.inp``
       writer.
   * - ``get_vertex_coupling_area(vertex_idx)`` /
       ``set_vertex_coupling_area(vertex_idx, area)``
     - Coupling exchange area in m² (``[2D_VERTEX_NODE_MAP]`` AREA
       column; default 1.0, must be > 0). Persisted by the ``.inp``
       writer.
...
.. list-table::
   ...
   * - ``get_vertex_render_depths()``
     ...
     - Render-oriented signed water depths at all vertices
       (``n_vertices``): the wet-masked, depth-weighted free-surface
       reconstruction ``eta_v - z_v`` (m). Unlike ``get_vertex_heads()``,
       dry-cell bed elevations never contribute; the value is negative
       over the dry side of a partially wet cell and ``0`` where no
       incident cell is wet. This is the field GUIs should interpolate
...
.. list-table::
   ...
   * - ``get_edge_bc_tseries_name`` / ``set_edge_bc_tseries_name``
     - Name the timeseries driving a SPECIFIED_STAGE edge; the getter
       returns ``""`` when the slot is clear.
   * - ``get_edge_bc_flow_tseries_name`` /
       ``set_edge_bc_flow_tseries_name``
     - Name the timeseries driving a SPECIFIED_FLOW edge; ``""`` when
       clear.
   * - ``get_edge_bc_rating_curve_name`` /
       ``set_edge_bc_rating_curve_name``
     - Name the rating curve driving a RATING_CURVE edge; ``""`` when
       clear.
