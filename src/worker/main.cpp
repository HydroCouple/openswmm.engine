/*!
 * \file main.cpp
 * \brief EPA SWMM 5.2.4 worker process for SWMMVis.
 *
 * Spawned by openswmm.gui's SimulationRunner with arguments:
 *   openswmm-legacy-worker-5.2.4 <inp_path> <rpt_path> <out_path> [tickIntervalMs]
 *
 * Each worker process has isolated global state, allowing true parallel
 * execution of 5.2.4 simulations without global-variable conflicts.
 *
 ...
 *
 * Port of openswmm.engine/src/legacy/worker/main.cpp (the 5.3.0 worker) onto
 * the stock 5.2.4 API plus the SWMMVis backports it needs
 * (swmm_setWarningCallback, swmm_getRunningMassBalErr, swmm_ENDDATE). The
 * control flow and every emitted line are identical to the 5.3.0 worker.
 */
...
// (set by the failing swmm_* call) into a human-readable string.
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4 || argc > 5) {
        WorkerProgress::emitError(1,
            "Usage: openswmm-legacy-worker-5.2.4 <inp_path> <rpt_path> <out_path> [tickIntervalMs]");
        return 1;
    }
    ...
    // the engine is single-threaded per process, so this is safe.
