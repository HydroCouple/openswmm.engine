...
void CvodeKokkosSurfaceSolver::uploadHeadToY() {
    copyToDevice(state_host_->head, y_->View());
    copyToDevice(state_host_->head, state_v_.head);
}

void CvodeKokkosSurfaceSolver::downloadState() {
    copyToHost(y_->View(), state_host_->head);
    // Depth = max(H − z, 0), recomputed on host exactly as the serial solver
    // does after CVode (CvodeSurfaceSolver::advance).
    for (int i = 0; i < nt_; ++i) {
        const double d = state_host_->head[i] - mesh_host_->tri_cz[i];
        state_host_->depth[i] = d > 0.0 ? d : 0.0;
    }
...
int CvodeKokkosSurfaceSolver::psetup_cb(sunrealtype /*t*/, N_Vector /*y*/,
    ...
    auto* c = static_cast<KokkosSolverContext*>(user_data);
    *jcurPtr = SUNTRUE;  // Phase 1 always rebuilds (mirrors serial path).
...
void CvodeKokkosSurfaceSolver::initialize(MeshData& mesh, SurfaceStateData& state,
    ...
    // Same option gate as the serial solver: GMRES + (NONE | JACOBI) only.
    ...
    if (opts.preconditioner != PreconditionerType::NONE &&
        ...
        throw std::runtime_error(
            "CvodeKokkosSurfaceSolver: only PRECONDITIONER=NONE or JACOBI is wired");
    }
    ...
    // Initial condition: y = head (SurfaceRouter2D sets head[i] = tri_cz[i]).
    uploadHeadToY();
    uploadSources();
    ...
    // Same tolerance collapse as the serial H-formulation path.
    const double atol_per_cell = opts.abs_tolerance
                               + opts.rel_tolerance * opts.dry_depth;
    if (CVodeSStolerances(cvode_mem_, 0.0, atol_per_cell) != CV_SUCCESS)
...
void CvodeKokkosSurfaceSolver::finalize() {
    ...
    y_.reset();  // destroy the Kokkos vector before its context
